...
# Copilot-specific session/break reports from the original token-finops-cli, kept verbatim.
from .copilot_legacy import (DEFAULT_GAP_MINUTES, connect_readonly, format_duration, format_tokens as format_tokens,  # noqa: F401
                             progress_bar, render_session_detail, render_sessions_totals,
                             session_breaks)
...
def cmd_report(args) -> str:
    ...
    for ad, rw, events in rows:
        if args.compact:
            ...
            lines += render_summary(f"{ad.display_name} ({label})",
...
def cmd_sessions(args) -> str:
    # Original Copilot behaviour: break/gap detail for one session, or totals across all.
    if args.session or args.totals:
        args.db_path = args.db_path or os.environ.get("TOKEN_FINOPS_DB",
                                                      os.path.expanduser("~/.copilot/session-store.db"))
        return render_session_detail(args) if args.session else render_sessions_totals(args)
    if args.db_path:
        os.environ["TOKEN_FINOPS_COPILOT_DB"] = args.db_path
        args.tool = ["copilot"]
    lines = []
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument("--session", default=None,
                   help="Copilot: detailed break/gap report for this one session_id instead of the list")
    s.add_argument("--gap-minutes", type=float, default=DEFAULT_GAP_MINUTES,
                   help=f"Copilot: idle gap (minutes) that counts as a break (default {DEFAULT_GAP_MINUTES:.0f})")
    s.add_argument("--totals", action="store_true",
                   help="Copilot: one combined report across ALL sessions (requests, tokens, AI units, active/idle time)")
    s.add_argument("--db-path", default=None,
                   help="Copilot session-store.db to read (default $TOKEN_FINOPS_DB or ~/.copilot/session-store.db)")
