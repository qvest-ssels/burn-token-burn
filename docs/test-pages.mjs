...
const PAGES = [
  { path: "index.html", player: "player-06", cast: "casts/06-install-pip.cast" },
  { path: "report.html", player: "player-01", cast: "casts/01-report.cast" },
  { path: "statusline.html", player: "player-02", cast: "casts/02-status-formats.cast" },
  { path: "self-audit.html", player: "player-03", cast: "casts/03-self-audit.cast" },
  { path: "savings.html", player: "player-04", cast: "casts/04-savings-break-even.cast" },
  { path: "synth.html", player: "player-05", cast: "casts/05-synth.cast" },
];
...
async function run() {
  ...
  for (const spec of PAGES) {
    ...
    const wrapper = page.locator(`#${spec.player}`);
    ...
    try {
      ...
      await page
        .locator(`#${spec.player} .ap-player.asciinema-player-theme-asciinema .ap-term canvas`)
        .first()
    ...
    check(`${spec.path}: player ${spec.player} initialised`, initialised);

    const box = await wrapper.boundingBox();
    check(`${spec.path}: player has a non-zero box`,
    ...
    check(`${spec.path}: player painted terminal content (not blank)`,
      painted.colours > 1,
    ...
    check(`${spec.path}: player width is min(60vw, 720px)`, widthOk,
      box ? `${Math.round(box.width)}px, expected ${expected}px` : "no box");

    const notFullBleed = !!box && box.width < DESKTOP.width * 0.75;
    check(`${spec.path}: player is not full-bleed`, notFullBleed,
    ...
    check(`${spec.path}: player is left-aligned with the prose`,
    ...
    // Navigation: six links, exactly one marked as the current page, and it
    // points at this page.
